...
<script>
/**
 * @thanks https://github.com/neuronetio/gantt-elastic
 */
...
import { notEqualDeep, mergeDeep, formatTask } from './utils';
...
export default defineComponent({
  ...
  methods: {
     ...
     */
    initialize() {
      let options = mergeDeep({}, this.state.options, getDefaultOptions(), this.options);
      dayjs.locale(options.locale, null, true);
      dayjs.locale(options.locale.name);

      let tasks = formatTask(this.tasks);
      ...
      this.state.options.outerHeight = this.state.options.height + useScrollbarHeight();
      this.globalOnResize();
    },
    ...
    calculateTaskListColumnsDimensions() {
      ...
      for (let column of this.state.options.taskList.columns) {
        ...
        column.height = this.getTaskHeight() - 1;
      }
     ...
     */
    initializeEvents() {
      emitter.on('chart-scroll-horizontal', this.onScrollChart);
      emitter.on('chart-scroll-vertical', this.onScrollChart);
      emitter.on('chart-wheel', this.onWheelChart);
      emitter.on('times-timeZoom-change', this.onTimeZoomChange);
      emitter.on('row-height-change', this.onRowHeightChange);
      emitter.on('scope-change', this.onScopeChange);
      emitter.on('taskList-width-change', this.onTaskListWidthChange);
      emitter.on('taskList-column-width-change', this.onTaskListColumnWidthChange);
    },
    ...
    recalculateTimes() {
      ...
      this.state.options.width =
        this.state.options.times.totalViewDurationPx + 1;
    },
  ...
  computed: {
    ...
    visibleTasks() {
      ...
      const safe = (n) => (isNaN(n) ? 0 : Math.max(n, 0));
      let len = visibleTasks.length;
      for (let index = 0; index < len; index++) {
        let task = visibleTasks[index];
        task.width =
          safe(task.duration / this.state.options.times.timePerPixel - 1);
        task.height = safe(this.state.options.row.height);
        task.x = safe(this.timeToPixelOffsetX(task.startTime));
        task.y =
          safe((this.state.options.row.height + this.state.options.chart.grid.horizontal.gap * 2) * index +
  ...
  mounted() {
    ...
    emitter.emit('gantt-elastic-mounted', this);
    emitter.emit('mounted', this);
    emitter.emit('gantt-elastic-ready', this);
  },

<template>
  <div class="gantt-elastic__calendar" :style="{ width: root.state.options.width + 'px' }">
    <calendar-row :items="dates.months" which="month" v-if="root.state.options.calendar.month.display" />
    <calendar-row :items="dates.days" which="day" v-if="root.state.options.calendar.day.display" />
    <calendar-row :items="dates.hours" which="hour" v-if="root.state.options.calendar.hour.display" />
  </div>
</template>
    ...
    generateDays() {
      ...
      for (let dayIndex = 0, len = steps.length; dayIndex < len; dayIndex += dayStep) {
        ...
        days.push({
          ...
          label: this.root.state.options.calendar.day.format[daysCount.type](date)
        });
      }
      ...
      for (let monthIndex = 0; monthIndex < monthsCount.count; monthIndex++) {
        ...
        for (let formatName of formatNames) {
          if (this.root.state.options.calendar.month.maxWidths[formatName] + 2 <= monthWidth) {
            label = this.root.state.options.calendar.month.format[formatName](currentDate);
...
.gantt-elastic__calendar {
  ...
  & > .gantt-elastic__calendar-row > .gantt-elastic__calendar-row-rect:last-child > .gantt-elastic__calendar-row-rect-child {
    border-right: none;
  }
}
</style>
